...
class BuyDipStrategy:
    def __init__(self, ib, quantity, diff, frame, logger):
        ...
        self.monitor_frame = StrategyMonitorFrame(
            None, self.last_price, self.last_price + self.diff if self.last_price else None, "Buy"
        )
    ...
    def _run(self):
        ...
        while self.active:
            ...
            if last_price is not None and last_price >= self.indic_price:
                ...
                order = create_limit_order("BUY", self.qty, round(self.indic_price, 2))
                trade = self.ib.placeOrder(contract, order)
                self.logger.add_entry(order.orderId, "UNH", self.qty, "BUY", last_price, self.indic_price, "pending")
                wx.CallAfter(
