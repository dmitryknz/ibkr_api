...
import logging
import json
from utils.order_utils import create_unh_contract
...
class IBapi(EWrapper, EClient):
    def __init__(self, frame):
        ...
        # --- Для накопления исторических баров ---
        self._minute_bars = []
    ...
    def nextValidId(self, orderId: int):
        if self.pending_limit_order:
            try:
                ...
                order.account = "U16306752"
                order.transmit = True  # Assign user account
    ...
    def historicalData(self, reqId, bar):
        try:
            # Логируем полученные бары
            logging.info(f"[HISTORICAL DATA] reqId={reqId}, bar={bar}")
            from datetime import datetime
            ts = bar.date if isinstance(bar.date, str) else str(bar.date)
            dt = datetime.strptime(ts, "%Y%m%d  %H:%M:%S") if len(ts) > 10 else datetime.strptime(ts, "%Y%m%d")
            if reqId == 1:
                # Копим минутные бары
                ...
                # Копим пятиминутные бары
                self._five_minute_bars.append((
                    dt,
                    bar.open,
                    bar.high,
                    bar.low,
            ...
            logging.exception(f"Ошибка в historicalData: {e}")

    def historicalDataEnd(self, reqId: int, start: str, end: str):
        """Сохраняем накопленные бары в базу одним вызовом"""
        try:
            if reqId == 1 and self._minute_bars:
                self.frame.data_manager.save_minute_data("UNH", self._minute_bars)
                print(f"Сохранено {len(self._minute_bars)} минутных баров (bulk)")
                self._minute_bars = []
            elif reqId == 2 and self._five_minute_bars:
                self.frame.data_manager.save_five_minute_data("UNH", self._five_minute_bars)
                print(f"Сохранено {len(self._five_minute_bars)} пятиминутных баров (bulk)")
                self._five_minute_bars = []
        except Exception as e:
            logging.exception(f"Ошибка в historicalDataEnd: {e}")
